...
async function main() {
  ...
  // 2. 删除PostgreSQL数据卷
  await runCommand(
    'docker volume rm -f captraw-documents_postgres_data',
    '删除PostgreSQL数据卷'
  );
  ...
  const dbCheck = await runCommand(
    'docker exec -i captraw-postgres psql -U captraw_user -d captraw_db -c "SELECT 1;"',
    '检查数据库连接'
  );
  ...
  console.log('\n📊 检查数据库表...');
  await runCommand(
    'docker exec -i captraw-postgres psql -U captraw_user -d captraw_db -c "\\dt"',
    '查看数据库表'
  );
