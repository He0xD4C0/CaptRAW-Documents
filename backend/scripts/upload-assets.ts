...
import { S3StorageClient } from '../src/storage/s3Client';
import { batchInsertAssets, AssetRecord as DBAssetRecord } from '../src/database';
...
// 编码联邦用户名
function encodeFediverseUsername(username: string, hostname: string): string {
  return encodeURIComponent(`@${username}@${hostname}`);
}

// 从配置构建对象键
function buildObjectKey(kind: AssetKind, uuid: string, username?: string, hostname?: string): string {
  if (kind === 'user') {
...
function loadExampleData() {
  const frontendDataPath = path.resolve(__dirname, '../../frontend/src/data');
  
  const articles: ArticleData[] = JSON.parse(
    fs.readFileSync(path.join(frontendDataPath, 'articles.json'), 'utf8')
  );
  
  const banners: BannerData[] = JSON.parse(
    fs.readFileSync(path.join(frontendDataPath, 'banners.json'), 'utf8')
  );
  
  const notices: NoticeData[] = JSON.parse(
    fs.readFileSync(path.join(frontendDataPath, 'notices.json'), 'utf8')
  );
...
// 生成资产元数据
function generateAssetMetadata(): AssetMetadata[] {
...
// 准备示例图片内容
async function prepareImageContent(): Promise<Buffer> {
  // 检查是否有示例图片文件
  const exampleImagePath = path.resolve(__dirname, '../../Reference/resource/useravatar-1024x1024.png');
...
async function main() {
  ...
  // 初始化S3客户端
  const s3Client = new S3StorageClient(config.objectStorage);
  ...
  // 上传资产到MinIO并插入数据库记录
  console.log('开始上传资产...');
  const dbAssets: Omit<DBAssetRecord, 'id' | 'created_at' | 'updated_at'>[] = [];
  ...
  console.log('插入数据库记录...');
  try {
    const inserted = await batchInsertAssets(dbAssets);
