...
async function main() {
    ...
    const seedDataPath = path_1.default.resolve(__dirname, '../seed-data.sql');
    ...
    const success = await runCommand(sqlCommand, '执行示例数据SQL文件');
    if (!success) {
    ...
    // 验证数据
    console.log('\n📊 验证数据填充结果...');
    await runCommand('docker exec -i captraw-postgres psql -U captraw_user -d captraw_db -c "SELECT \\\'Users\\\' as table_name, COUNT(*) as count FROM users UNION ALL SELECT \\\'Articles\\\', COUNT(*) FROM articles UNION ALL SELECT \\\'Announcements\\\', COUNT(*) FROM announcements UNION ALL SELECT \\\'Banners\\\', COUNT(*) FROM banners UNION ALL SELECT \\\'Server Info\\\', COUNT(*) FROM server_info;"', '查看各表数据量');
    await runCommand('docker exec -i captraw-postgres psql -U captraw_user -d captraw_db -c "SELECT info_key, info_value::text, description FROM server_info ORDER BY category, info_key LIMIT 5;"', '查看服务器配置示例');
    await runCommand('docker exec -i captraw-postgres psql -U captraw_user -d captraw_db -c "SELECT a.article_title, u.nickname as author, a.tags, a.views, a.likes FROM articles a JOIN users u ON a.author_id = u.user_uuid ORDER BY a.release_time DESC LIMIT 3;"', '查看文章示例');
    console.log('\n🎉 数据库示例数据填充完成！');
    console.log('\n📋 数据统计:');
